...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/03 19:40:02 by iarbaiza          #+#    #+#             */
/*   Updated: 2023/11/13 12:37:53 by iarbaiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_dead(t_list *lst)
{
	...
	{
		time = get_time();
		meal_time = get_last_meal(lst, time, i);
...
void	check_dead_meals(t_list *lst)
{
	...
	while (1)
	{
		...
		meal_time = get_last_meal(lst, time, i);
		if (meal_time > lst->time_die
			&& lst->num_meals == 0 && finish_eating(lst) == 1)
		{
			philo_dead(lst, time, i);
			return ;
		}
